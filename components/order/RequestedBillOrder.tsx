...
export default function RequestedBillOrder({ dailyOrder }: RequestedBillOrderProps) {
    ...
    <div className="bg-white shadow rounded-lg flex flex-col h-full">
      <div className="p-5 space-y-4">
        ...
        {dailyOrder.orders.map((order, index) => (
          <div key={order.id} className="border-t pt-4 first:border-t-0 first:pt-0">
            ...
            <ul className="divide-y divide-gray-200 text-sm font-medium text-gray-500">
              {order.orderProducts.map(product => (
                <li key={product.id} className="flex justify-between py-2 text-lg">
                  ...
                  <p>{formatCurrency((product.product.price) * product.quantity)}</p>
                </li>
              ))}
            </ul>
      ...
      <div className="mt-auto p-5 pt-0 space-y-4">
        <div className="border-t pt-4">
          <p className="text-2xl font-bold text-slate-600 text-right mt-4">
            Total: {formatCurrency(dailyOrder.total)}
        <p className="text-2xl font-bold text-slate-600 capitalize  text-right mt-4 ">
